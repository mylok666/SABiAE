...
class ConvLSTM(nn.Module):
    ...
    def forward(self, input_tensor):
        ...
        for layer_idx in range(self.num_layers):  # for layer_idx in range(5):
            output_inner = []
            forward_states = []
            ...
            for t in range(seq_len):
                if t == 0:
                    ...
                    i = 384
                hf, cf = self.cell_list[layer_idx](input_tensor=cur_layer_input[:, i:i + 128, :, :],
                                                   cur_state=[hf, cf])  # hf.shape torch.Size([1, 128, 32, 32])

                forward_states.append(hf)
                #print(len(forward_states))

            layer_output = torch.cat(forward_states, dim=1)
