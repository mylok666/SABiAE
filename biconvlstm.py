...
class BiConvLSTMCell(nn.Module):
    ...
    def forward(self, input_tensor, cur_state):
        ...
        #print('hhhhh====',h_cur.shape)
        #print('iiiiin====',input_tensor.shape)
...
class BiConvLSTM(nn.Module):
    ...
    def __init__(self, input_size, input_dim, hidden_dim, kernel_size, num_layers,
        ...
        kernel_size = self._extend_for_multilayer(kernel_size, num_layers)  #[(3,3),(3,3),(3,3),(3,3),(3,3)]
    ...
    def forward(self, input_tensor):
        ...
        seq_len = input_tensor.size(1) //128  # torch.Size([8, 6, 256, 256])中的8 就是你的batch_size input_tensor.size(1) = 6
        cur_layer_input = input_tensor  #就是dataloader里的每一组图 torch.Size([8, 6, 256, 256])
        #print('cur===', cur_layer_input.shape) #.Size([1, 512, 32, 32])
        for layer_idx in range(self.num_layers):
            backward_states = []
            forward_states = []
            output_inner = []
            #print('curalin===', cur_layer_input.shape) #.Size([1, 512, 32, 32])
            hb, cb = hidden_state[layer_idx]  # torch.Size([1, 128, 32, 32])
            for t in range(seq_len):
                if t == 0:
                    ...
                    i = 0
                hb, cb = self.cell_list[layer_idx](input_tensor=cur_layer_input[:, i:i+128 , :, :], cur_state=[hb, cb])
            ...
            for t in range(seq_len):
                if t == 0:
                    ...
                    i = 384
                hf, cf = self.cell_list[layer_idx](input_tensor=cur_layer_input[:, i:i+128, :, :], cur_state=[hf, cf]) #hf.shape torch.Size([1, 128, 32, 32])
                #hf, cf = self.cell_list[layer_idx](input_tensor=cur_layer_input[:, t, :, :], cur_state=[hf, cf])
                forward_states.append(hf)

            for t in range(seq_len):
                h = self.cell_list[layer_idx].conv_concat(torch.cat((forward_states[t], backward_states[seq_len - t - 1]), dim=1)) #.Size([1, 128, 32, 32])
                #h = self.attn(h)
                output_inner.append(h)
                #print('No.{}:outppppt.length ==={}'.format(t+1,len(output_inner)))
            #print('outppppt.length ===',len(output_inner)) #512
